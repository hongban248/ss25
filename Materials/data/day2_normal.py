...
def log_likelihood(data, mu, sigma):
    n = len(data)
    log_likelihood_value = -0.5 * n * np.log(2 * np.pi) -  n * np.log(sigma) - 0.5 / (sigma**2) * np.sum((data - mu)**2)
...
def index2(ori,n):
    #start=ori*0.1
    fendu=ori*1.2/n
    return [ori*+0.6+fendu*i for i in range(n)]
...
for i in range(result.shape[0]):
    ...
    for j in range(result.shape[1]):
        result[i][j]=log_likelihood(data=data,mu=index(-325.98,result.shape[0])[i],sigma=index2(8.57,result.shape[1])[j])
...
for i in range(max_iterations):
    l0=log_likelihood(data=data,mu=x_labels[x],sigma=y_labels[y])
    #print(l0)
    ...
    l1 = log_likelihood(data=data, mu=x_labels[new_x], sigma=y_labels[new_y])
